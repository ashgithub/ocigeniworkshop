"""
What this file does:
Demonstrates automatic multi-step function calling with a LangChain agent using
OCI Generative AI through the OpenAI-compatible client.
...
How to run the file:
uv run langChain/function_calling/langchain_multi_auto.pyuv run langChain/function_calling/langchain_multi_auto.py

Important sections:
- Step 1: Load the configuration
- Step 2: Create the LLM client
- Step 3: Create the agent
- Step 4: Invoke and stream the multi-step agent response
"""
...
agent = create_agent(
    llm_client,
    tools=tools,
    system_prompt="Use one or more tools to get a response. Once you have the answer, reply in Hinglish (Hindi mixed with English).",
)
