"""
What this file does:
Oracle Cloud Infrastructure – Speech-to-Text (STT) helper script using Whisper model. Uploads audio to Object Storage, submits transcription job, polls for completion, downloads results (TXT + optional SRT).
...
How to run the file:
uv run speech/oci_speech_stt_whisper.py --file speech/voice_sample_english.mp3
...
- Since Whisper auto-detects language, try audio in different languages.
...
def create_transcription_job(
    ...
    model_details = oci.ai_speech.models.TranscriptionModelDetails(
        language_code="auto",
        model_type="WHISPER_MEDIUM",  # or ORACLE when US-IAD region
        domain="GENERIC",
...
def main() -> None:  # noqa: C901
    ...
    parser.add_argument(
        ...
        help=f"Sandbox YAML path (default: {SANDBOX_CONFIG_FILE})",
    )
    parser.add_argument(
        "--formats",
        nargs="*",
